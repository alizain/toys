...
const sessionPath = process.argv[2] || '/Users/alizain/.claude/projects/-Users-alizain-803-web-packages-toys/eb94088a-a782-4bf4-a57c-c486019f0c53.jsonl';
...
for (const line of lines) {
  try {
    ...
    if (obj.type === 'assistant') {
      ...
      if (Array.isArray(cnt)) {
        for (const item of cnt) {
          if (item.type === 'text' && item.text && item.text.length > 50 && assistantTexts.length < 30) {
...
console.log('\n=== TOP TOOLS USED ===');
const counts = {};
toolUses.forEach(t => counts[t] = (counts[t] || 0) + 1);
